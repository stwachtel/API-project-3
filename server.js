...
const routes = {
    ...
    POST: function(url,request) {
      ...
      if (request.body) {
        if (request.body.comment) {
          if (database.users[request.body.comment.username] && database.articles[request.body.comment.articleId]) {
            if (request.body.comment.username && request.body.comment.body &&
              ...
              let newComment = {
                ...
                database.users[newComment.username].commentIds.push(database.nextCommentId);
                database.articles[newComment.articleId].commentIds.push(database.nextCommentId);
                response.body={};
                response.body.comment={};
                response.status = 201;
                response.body.comment.id = newComment.id;
                response.body.comment.body = newComment.body;
                response.body.comment.username = newComment.username;
                response.body.comment.articleId = newComment.articleId;
                response.body.comment.upvotedBy = [];
                response.body.comment.downvotedBy = [];
  ...
  '/comments/:id': {
    PUT: function(url,request) {
      updatedCommentId = url.split('/')[2];
      const response={
        status: ''
      };
      if (request.body) {
        if (request.body.comment) {
          if (database.comments[updatedCommentId]) {
          const updatedComment = {
            ...
            articleId: request.body.comment.articleId,
                ...
                database.comments[updatedComment.id] = updatedComment;
                response.body={};
                response.body.comment={};
                response.status = 200;
                response.body.comment.id = updatedComment.id;
                if (updatedComment.body != '') {
                  response.body.comment.body = updatedComment.body;
                  } else {
                  response.body.comment.body = 'Nope';
                  database.comments[updatedComment.id].body = 'Nope';
                }
                response.body.comment.username = updatedComment.username;
                response.body.comment.articleID = updatedComment.articleId;
                response.body.comment.upvotedBy = [];
                response.body.comment.downvotedBy = [];
              } else {
                response.status = 404;
    ...
    DELETE: function(url) {
      ...
      deletedCommentId = url.split('/')[2];

      if (database.comments[deletedCommentId]) {

        deletedCommentUser = database.comments[deletedCommentId].username;
        userComments = database.users[deletedCommentUser].commentIds;
        remainingUserComments = [userComments.pop(deletedCommentId)];
        database.users[deletedCommentUser].commentIds = remainingUserComments;

        deletedCommentArticle = database.comments[deletedCommentId].articleId;
        articleComments = database.articles[deletedCommentArticle].commentIds;
        remainingArticleComments = [articleComments.pop(deletedCommentId)];
        database.articles[deletedCommentArticle].commentIds = remainingArticleComments;

        database.comments[deletedCommentId] = null;
  ...
  '/comments/:id/upvote': {
    PUT: function(url,request) {
      ...
      if (request.body) {
        ...
        if (savedComment && database.users[upvoteUser]) {
          savedComment = upvote(savedComment, upvoteUser)
  ...
  '/comments/:id/downvote': {
    PUT: function(url,request) {
      ...
      if (request.body) {
        ...
        let savedComment = database.comments[id];
        if (savedComment && database.users[downvoteUser]) {
          savedComment = downvote(savedComment, downvoteUser)
          response.body = {comment: savedComment};
          response.status = 200;
        } else {
          response.status = 400;
